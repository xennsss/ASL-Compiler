...
antlrcpp::Any TypeCheckVisitor::visitArithmetic(AslParser::ArithmeticContext *ctx) {
  ...
  if (Types.isFloatTy(t1) or Types.isFloatTy(t2)) TypesMgr::TypeId t = Types.createFloatTy();
  else TypesMgr::TypeId t = Types.createIntegerTy();
  putTypeDecor(ctx, t);
...
antlrcpp::Any TypeCheckVisitor::visitUnitaryMinus(AslParser::UnitaryMinusContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->expr());
  TypesMgr::TypeId t1 = getTypeDecor(ctx->expr());
  if ((not Types.isErrorTy(t1)) and (not Types.isNumericTy(t1)))
    Errors.incompatibleOperator(ctx->op);
  if (Types.isFloatTy(t1)) TypesMgr::TypeId t = Types.createFloatTy();
  else TypesMgr::TypeId t = Types.createIntegerTy();
  putTypeDecor(ctx, t);
  putIsLValueDecor(ctx, false);
  DEBUG_EXIT();
...
antlrcpp::Any TypeCheckVisitor::visitParentesis(AslParser::ParentesisContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->expr());
  DEBUG_EXIT();
  return 0;
